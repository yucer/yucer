# Software & Infrastructure Engineer

I build and maintain software and infrastructure with a focus on reliability, automation, and maintainability.

My work spans backend development, Linux systems, APIs, databases, and open-source technologies. I'm particularly interested in software architecture, semantic computing, and designing systems that are simple to understand, easy to operate, and built to evolve.

---

## Current Focus

- 🐧 Building reliable Linux infrastructure
- ⚙️ Improving automation workflows
- 🏗️ Developing backend services and APIs
- 🧠 Exploring semantic computing and the practical use of AI
- 🌍 Contributing to open-source software
- 📚 Learning new technologies through practical projects

---

## Core Technologies

<p align="center">
      <img src="https://www.vectorlogo.zone/logos/linux/linux-icon.svg" alt="Linux" width="55" height="55"/>
      <img src="https://www.vectorlogo.zone/logos/python/python-icon.svg" alt="Python" width="55" height="55"/>
      <img src="https://www.vectorlogo.zone/logos/docker/docker-icon.svg" alt="Docker" width="55" height="55"/>
      <img src="https://www.vectorlogo.zone/logos/git-scm/git-scm-icon.svg" alt="Git" width="55" height="55"/>
      <img src="https://www.vectorlogo.zone/logos/github/github-tile.svg" alt="GitHub" width="55" height="55"/>
      <img src="https://www.vectorlogo.zone/logos/visualstudio_code/visualstudio_code-icon.svg" alt="VS Code" width="55" height="55"/>
      <img src="https://www.vectorlogo.zone/logos/jupyter/jupyter-icon.svg" alt="Jupyter" width="55" height="55"/>
      <img src="https://www.vectorlogo.zone/logos/amazon_aws/amazon_aws-icon.svg" alt="AWS" width="55" height="55"/>
</p>

---

## Projects

I'm gradually publishing projects that reflect my interests in software engineering, Linux infrastructure, automation, semantic computing, and the practical use of AI.

---

## GitHub Stats

<p align="center">
  <img height="170" src="./profile/stats/github-stats.svg" alt="GitHub statistics card for yucer" />
  <img height="170" src="./profile/stats/top-languages.svg" alt="Most used languages card for yucer" />
</p>
